...
`명조: 워더링 웨이브`의 에이메스(Aemeath)에서 영감을 받아 만든 비공식 Codex 커스텀 펫입니다. Codex Desktop의 pet overlay에서 사용할 수 있는 `pet.json`과 `spritesheet.webp`를 제공합니다.
...
에이메스는 KURO GAMES의 오픈월드 액션 RPG `명조: 워더링 웨이브`에 등장하는 공명자입니다. 이 저장소의 펫은 해당 캐릭터를 개인 Codex 환경에서 사용할 수 있도록 재해석한 팬메이드 커스텀 avatar 패키지입니다.

이 저장소는 원작 게임, 원작사, 공식 배포 채널과 관련이 없습니다.
...
`명조: 워더링 웨이브`, 에이메스, 관련 캐릭터 디자인, 게임명, 상표, 저작권은 각 권리자에게 있습니다. 이 저장소는 비공식 팬메이드 커스텀 펫 배포용이며, 원작사와 관련이 없고 공식 배포물이 아닙니다.
